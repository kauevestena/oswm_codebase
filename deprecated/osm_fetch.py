...
def filter_gjsonfeats_bygeomtype(
    geojson, geomtype="LineString", lvl1="features", include_feats_without_tags=False
):
    """
...
def get_osm_data(
    ...
    geojson_datadict = osm2geojson.xml2geojson(
        xml_filecontent, filter_used_refs=False, log_level="INFO"
    )

    if not return_as_string:
        with open(
            geojsonfilepath.replace(".geojson", "_unfiltered.geojson"), "w+"
        ) as geojson_handle:

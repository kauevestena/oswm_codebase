...
from oswm_codebase.deprecated.webmap_insertions import *
...
sidewalks_gdf = sidewalks_gdf.reindex(
    columns=req_fields["sidewalks"] + mandatory_fields, fill_value="?"
)
crossings_gdf = crossings_gdf.reindex(
    columns=req_fields["crossings"] + mandatory_fields, fill_value="?"
)
kerbs_gdf = kerbs_gdf.reindex(
    columns=req_fields["kerbs"] + mandatory_fields, fill_value="?"
)
...
for category in gdf_dict:
    if category == "kerbs":
        gdf_dict[category]["osm_id"] = (
            gdf_dict[category]["id"].astype("string").apply(return_weblink_node)
        )
    else:
        gdf_dict[category]["osm_id"] = (
            gdf_dict[category]["id"].astype("string").apply(return_weblink_way)
        )
...
# standard:
std_baselayer = folium.TileLayer(
...
# opvnkarte:
opvnkarte_baselayer = folium.TileLayer(
    tiles="https://tile.memomaps.de/tilegen/{z}/{x}/{y}.png",
    max_zoom=22,
    max_native_zoom=18,
    name="OPVN Karte Transport",
    opacity=0.5,
    attr="OPVN Karte Transport",
    min_zoom=min_zoom,
)  # .add_to(m)
...
# for 'crossing' tag
crossing_colors = get_attr_dict(
    fields_values_properties, category="crossings", osm_tag="crossing"
)  # {}

# dasharray:
crossing_dash = get_attr_dict(
    fields_values_properties, category="crossings", osm_tag="crossing", attr="dasharray"
)  # {}

# dashoffset:
crossing_dashoffset = get_attr_dict(
...
def style_crossing(feature):
    # real thanks to: https://gis.stackexchange.com/questions/394219/folium-draw-polygons-with-distinct-colours
    return {
        "color": feature["properties"]["crossing_colors"],
        "weight": 4,
        "dashArray": feature["properties"]["crossing_dash"],
        "dashOffset": feature["properties"]["crossing_dashoffset"],
        "opacity": 0.6,
    }
...
sidewalk_surface_layer = folium.GeoJson(
    data=sidewalks_gdf
    # sidewalks_path
    ,
    ...
    tooltip=folium.Tooltip(
        "<b>Sidewalk</b><br>(click for details)", "text-align:center"
    ),
)  # .add_to(m)

sidewalk_smoothness_layer = folium.GeoJson(
    data=sidewalks_gdf
    # sidewalks_path
    ,
...
crossings_layer = folium.GeoJson(
    data=crossings_gdf,
    name="crossings",
    ...
    # tooltip='Crossing',
    tooltip=folium.Tooltip(
        "<b>Crossing</b><br>(click for details)", "text-align:center"
    ),
)  # .add_to(m)
...
kerbs_colors = get_attr_dict(
    fields_values_properties, category="kerbs", osm_tag="kerb"
)  # {}
...
def styling_kerbs(feature):
    return {
        "color": "black",
        "fillColor": feature["properties"]["kerbs_colors"],
        "fillOpacity": 1,
    }


kerbs_layer = folium.GeoJson(
    data=kerbs_gdf,
    name="kerbs",
    marker=folium.Circle(radius=1, fill=True, weight=1),
...
kerbs_tactilepaving_opacity = get_attr_dict(
    fields_values_properties, category="kerbs", osm_tag="tactile_paving", attr="opacity"
)  # {}

# print(kerbs_tactilepaving_colors)

kerbs_gdf["kerbs_tactilepaving_opacity"] = kerbs_gdf["tactile_paving"]
kerbs_gdf["kerbs_tactilepaving_opacity"].replace(
    kerbs_tactilepaving_opacity, inplace=True
)
...
def styling_kerbs_tac_paving(feature):
    return {
        "opacity": 0,
        "fillColor": "black",
        "fillOpacity": feature["properties"]["kerbs_tactilepaving_opacity"],
    }
...
folium.GeoJson(
    ...
    name="kerbs_tp",
    control=False,
    marker=folium.Circle(radius=0.4, fill=True, weight=0.1),
...
def tp_sw_style(feature):
    return {"color": "black", "weight": 0.5, "opacity": 0.7}


sw_tp_gdf = sidewalks_gdf.loc[
    sidewalks_gdf.set_index(["tactile_paving"]).index.isin(["yes", "contrasted"])
]
...
GroupedLayerControl(
    groups={
        "Sidewalks Layers": [sidewalk_surface_layer, sidewalk_smoothness_layer],
        "Crossings Layers": [crossings_layer],
        "Kerbs Layers": [kerbs_layer],
        "Base Layers": [std_baselayer, opvnkarte_baselayer],
    },
    collapsed=True,
).add_to(m)
...
logo_path = "https://kauevestena.github.io/oswm_codebase/assets/are_sidewalks_acessible_typog.png"

float_image_1 = FloatImage(logo_path, bottom=0.5, left=0.5).add_to(m)

footer_path = "https://kauevestena.github.io/oswm_codebase/assets/footer.png"

float_image_2 = FloatImage(footer_path, bottom=0.5, left=0).add_to(m)
...
logo_ref = find_html_name(map_page_name, logo_path)
style_changer(map_page_name, logo_ref)


# applying a rule to hide footer message for small screens
footer_ref = find_html_name(map_page_name, footer_path)
...
style_changer(
    map_page_name, find_map_ref(map_page_name), original="relative", new="fixed"
)
...
for insertion_point in insertions_dict:
    ...
    # add_to_page_after_first_tag(map_page_name,insertions_dict[insertion_point],insertion_point)

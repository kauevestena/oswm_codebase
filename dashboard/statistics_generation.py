...
chart_global_insertions = get_dashboard_global_insertions(levels_backward=2)
for category in generated_list_dict:
    for i, rel_path in enumerate(generated_list_dict[category]):
        ...
        for insertpoint in chart_global_insertions:
            fileObj.simple_replace(insertpoint, chart_global_insertions[insertpoint])
        ...
        # Replace absolute node_homepage_url with relative path "../../"
        file_topbar = category_bars[category].replace(node_homepage_url, "../../")
        fileObj.simple_replace("<head>", "<head>\n" + file_topbar)

        # prepend warning comment
        warning_comment = "<!--\n  Generated automatically by oswm_codebase/dashboard/statistics_generation.py\n  Do not edit this file directly.\n-->\n"
        fileObj.content = warning_comment + fileObj.content
...
index_global_insertions = get_dashboard_global_insertions(levels_backward=1)
for insertpoint in index_global_insertions:
    fileObj.simple_replace(insertpoint, index_global_insertions[insertpoint])

# Replace absolute node_homepage_url with relative path "../" for the index file
index_topbar = topbar.replace(node_homepage_url, "../")
fileObj.simple_replace("<head>", "<head>\n" + index_topbar)

dashboard_main_page_insertions = get_dashboard_main_page_insertions()
